...
describe('LiquidatorProxyV1WithAmm', () => {
  beforeAll(async () => {
    ...
    expect(await dolomiteMargin.testing.uniswapV2Factory.getPairInitCodeHash()).toEqual(
...
async function liquidate(
  ...
  if (expiry && expiry.gt(INTEGERS.ZERO)) {
    expect(logs.filter(log => log.name === 'LogLiquidate').length).toEqual(0);
    // the expiration is a trade, so add one back in
    expect(logs.filter(log => log.name === 'LogTrade').length).toEqual(tokenPath.length - 1 + 1);
  } else {
    expect(logs.filter(log => log.name === 'LogLiquidate').length).toEqual(1);
    expect(logs.filter(log => log.name === 'LogTrade').length).toEqual(tokenPath.length - 1);
  }
  return txResult;
}
