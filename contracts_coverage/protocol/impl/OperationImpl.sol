...
library OperationImpl {
    ...
    function _verifyInputs(
        ...
        pure
    {
        if (accounts.length != 0) { /* FOR COVERAGE TESTING */ }
        Require.that(accounts.length != 0,
            FILE,
            "Cannot have zero accounts"
        );
        if (actions.length != 0) { /* FOR COVERAGE TESTING */ }
        Require.that(actions.length != 0,
            FILE,
        ...
        for (uint256 a = 0; a < accounts.length; a++) {
            for (uint256 b = a + 1; b < accounts.length; b++) {
                if (!Account.equals(accounts[a], accounts[b])) { /* FOR COVERAGE TESTING */ }
                Require.that(!Account.equals(accounts[a], accounts[b]),
                    FILE,
        ...
        )
    {
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (accountLayout != Actions.AccountLayout.OnePrimary) {
                if (arg.accountId != arg.otherAccountId) { /* FOR COVERAGE TESTING */ }
                Require.that(arg.accountId != arg.otherAccountId,
                    FILE,
                ...
                if (accountLayout == Actions.AccountLayout.TwoPrimary) {
                    ...
                    if (!primaryAccounts[arg.otherAccountId]) { /* FOR COVERAGE TESTING */ }
                    Require.that(!primaryAccounts[arg.otherAccountId],
                        FILE,
            ...
            if (marketLayout == Actions.MarketLayout.OneMarket) {
                ...
                if (arg.primaryMarketId != arg.secondaryMarketId) { /* FOR COVERAGE TESTING */ }
                Require.that(arg.primaryMarketId != arg.secondaryMarketId,
                    FILE,
        ...
        private
    {
        ...
        for (uint256 a = 0; a < accounts.length; a++) {
            ...
            if (collateralized) { /* FOR COVERAGE TESTING */ }
            Require.that(collateralized,
                FILE,
        ...
        private view
    {
        ...
        if (actualNumberOfMarketsWithBalances <= cachedNumberOfMarketsWithBalances ||actualNumberOfMarketsWithBalances <= state.riskParams.accountMaxNumberOfMarketsWithBalances) { /* FOR COVERAGE TESTING */ }
        Require.that(actualNumberOfMarketsWithBalances <= cachedNumberOfMarketsWithBalances ||
