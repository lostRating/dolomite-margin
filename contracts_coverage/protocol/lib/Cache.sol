...
library Cache {
    ...
    {
        if (cache.markets.length > 0) { /* FOR COVERAGE TESTING */ }
        Require.that(cache.markets.length > 0,
            FILE,
            "not initialized"
        );
        return _getInternal(
    ...
    function getAtIndex(
        ...
        returns (MarketInfo memory)
    {
        if (index < cache.markets.length) { /* FOR COVERAGE TESTING */ }
        Require.that(index < cache.markets.length,
            FILE,
            "invalid index",
            index,
