...
contract GLPPriceOracleV1 is IPriceOracle {
    ...
    function updateOraclePrice() external {
        ...
        if (timeElapsed >= UPDATE_DURATION) { /* FOR COVERAGE TESTING */ }
        Require.that(timeElapsed >= UPDATE_DURATION,
            FILE,
    ...
    returns (Monetary.Price memory) {
        ...
        if (_oraclePrice != 0) { /* FOR COVERAGE TESTING */ }
        Require.that(_oraclePrice != 0,
            FILE,
            "oracle price not set"
        );
        if (token == glp || token == fGlp) { /* FOR COVERAGE TESTING */ }
        Require.that(token == glp || token == fGlp,
            FILE,
            "invalid token"
        );
        if (block.timestamp.sub(lastOraclePriceUpdateTimestamp) < EXPIRATION_DURATION) { /* FOR COVERAGE TESTING */ }
        Require.that(block.timestamp.sub(lastOraclePriceUpdateTimestamp) < EXPIRATION_DURATION,
            FILE,
