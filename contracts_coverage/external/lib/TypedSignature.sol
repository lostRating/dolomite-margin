...
library TypedSignature {
    ...
    function recover(
        ...
        returns (address)
    {
        if (signatureWithType.length == NUM_SIGNATURE_BYTES) { /* FOR COVERAGE TESTING */ }
        Require.that(signatureWithType.length == NUM_SIGNATURE_BYTES,
            FILE,
        ...
        if (rawSigType < uint8(SignatureType.Invalid)) { /* FOR COVERAGE TESTING */ }
        Require.that(rawSigType < uint8(SignatureType.Invalid),
            FILE,
