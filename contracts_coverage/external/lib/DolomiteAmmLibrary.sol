...
library DolomiteAmmLibrary {
    ...
    bytes32 private constant PAIR_INIT_CODE_HASH = 0xb26142e3e3c9f33d5df6a084718f04380d76b81ea6932bc8782478cbb6115827;
    ...
    function getPools(
        ...
        if (path.length >= 2) { /* FOR COVERAGE TESTING */ }
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        if (tokenA != tokenB) { /* FOR COVERAGE TESTING */ }
        Require.that(tokenA != tokenB,
            FILE,
            "identical addresses"
        );
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        if (token0 != address(0)) { /* FOR COVERAGE TESTING */ }
        Require.that(token0 != address(0),
            FILE,
            "zero address"
        );
    }
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        if (amountA > 0) { /* FOR COVERAGE TESTING */ }
        Require.that(amountA > 0,
            FILE,
            "insufficient amount"
        );
        if (reserveA > 0 && reserveB > 0) { /* FOR COVERAGE TESTING */ }
        Require.that(reserveA > 0 && reserveB > 0,
            FILE,
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        if (amountIn > 0) { /* FOR COVERAGE TESTING */ }
        Require.that(amountIn > 0,
            FILE,
            "insufficient input amount"
        );
        if (reserveIn > 0 && reserveOut > 0) { /* FOR COVERAGE TESTING */ }
        Require.that(reserveIn > 0 && reserveOut > 0,
            FILE,
            "insufficient liquidity"
        );
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        if (amountOut > 0) { /* FOR COVERAGE TESTING */ }
        Require.that(amountOut > 0,
            FILE,
            "insufficient output amount"
        );
        if (reserveIn > 0 && reserveOut > 0 && reserveOut > amountOut) { /* FOR COVERAGE TESTING */ }
        Require.that(reserveIn > 0 && reserveOut > 0 && reserveOut > amountOut,
            FILE,
            "insufficient liquidity"
        );
