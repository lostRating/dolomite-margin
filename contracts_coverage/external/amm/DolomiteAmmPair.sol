...
contract DolomiteAmmPair is IDolomiteAmmPair, DolomiteAmmERC20, IAutoTrader {
    ...
    function initialize(address _token0, address _token1, address _transferProxy) external {
        if (msg.sender == factory) { /* FOR COVERAGE TESTING */ }
        Require.that(msg.sender == factory,
            FILE,
            "forbidden"
        );
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        if (amount0 > 0) { /* FOR COVERAGE TESTING */ }
        Require.that(amount0 > 0,
            FILE,
            "invalid mint amount 0"
        );
        if (amount1 > 0) { /* FOR COVERAGE TESTING */ }
        Require.that(amount1 > 0,
            FILE,
        ...
        if (liquidity > 0) { /* FOR COVERAGE TESTING */ }
        Require.that(liquidity > 0,
            FILE,
    ...
    function burn(address to, uint toAccountNumber) external lock returns (uint amount0Wei, uint amount1Wei) {
        ...
        /* solium-disable indentation */
        {
            ...
            if (amount0Wei > 0 && amount1Wei > 0) { /* FOR COVERAGE TESTING */ }
            Require.that(amount0Wei > 0 && amount1Wei > 0,
                FILE,
    ...
    returns (Types.AssetAmount memory) {
        ...
        if (msg.sender == address(cache.dolomiteMargin)) { /* FOR COVERAGE TESTING */ }
        Require.that(msg.sender == address(cache.dolomiteMargin),
            FILE,
            "invalid sender"
        );
        if (makerAccount.owner == address(this)) { /* FOR COVERAGE TESTING */ }
        Require.that(makerAccount.owner == address(this),
            FILE,
            "invalid maker account owner"
        );
        if (makerAccount.number == 0) { /* FOR COVERAGE TESTING */ }
        Require.that(makerAccount.number == 0,
            FILE,
            "invalid maker account number"
        );
        if (token0 != takerAccount.owner && token1 != takerAccount.owner && address(this) != takerAccount.owner) { /* FOR COVERAGE TESTING */ }
        Require.that(token0 != takerAccount.owner && token1 != takerAccount.owner && address(this) != takerAccount.owner,
            FILE,
        ...
        /* solium-disable indentation */
        {
            if (inputMarketId == cache.marketId0 || inputMarketId == cache.marketId1) { /* FOR COVERAGE TESTING */ }
            Require.that(inputMarketId == cache.marketId0 || inputMarketId == cache.marketId1,
                FILE,
                "invalid input market"
            );
            if (outputMarketId == cache.marketId0 || outputMarketId == cache.marketId1) { /* FOR COVERAGE TESTING */ }
            Require.that(outputMarketId == cache.marketId0 || outputMarketId == cache.marketId1,
                FILE,
                "invalid output market"
            );
            if (inputWei.sign) { /* FOR COVERAGE TESTING */ }
            Require.that(inputWei.sign,
                FILE,
            ...
            if (amountOutWei > 0) { /* FOR COVERAGE TESTING */ }
            Require.that(amountOutWei > 0,
                FILE,
        ...
        /* solium-disable indentation */
        {
            ...
            if (amount0OutWei < _reserve0 && amount1OutWei < _reserve1) { /* FOR COVERAGE TESTING */ }
            Require.that(amount0OutWei < _reserve0 && amount1OutWei < _reserve1,
                FILE,
            ...
            if (amount0InWei > 0 || amount1InWei > 0) { /* FOR COVERAGE TESTING */ }
            Require.that(amount0InWei > 0 || amount1InWei > 0,
                FILE,
            ...
            if (balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000 ** 2)) { /* FOR COVERAGE TESTING */ }
            Require.that(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000 ** 2),
                FILE,
                "K"
            );
    ...
    function _update(
        ...
        if (balance0 <= uint112(- 1) && balance1 <= uint112(- 1)) { /* FOR COVERAGE TESTING */ }
        Require.that(balance0 <= uint112(- 1) && balance1 <= uint112(- 1),
            FILE,
            "balance overflow"
        );
