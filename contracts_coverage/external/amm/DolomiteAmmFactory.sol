...
contract DolomiteAmmFactory is IDolomiteAmmFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        if (token0 != address(0) && token1 != address(0)) { /* FOR COVERAGE TESTING */ }
        Require.that(token0 != address(0) && token1 != address(0),
            FILE,
            "zero address"
        );
        if (getPair[token0][token1] == address(0)) { /* FOR COVERAGE TESTING */ }
        Require.that(getPair[token0][token1] == address(0),
            FILE,
            "pair already exists"
        );
    ...
    function setFeeTo(address _feeTo) external {
        if (msg.sender == feeToSetter) { /* FOR COVERAGE TESTING */ }
        Require.that(msg.sender == feeToSetter,
            FILE,
            "forbidden"
        );
        feeTo = _feeTo;
