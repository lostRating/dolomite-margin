...
contract PayableProxy is OnlyDolomiteMargin, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (owner1 == msg.sender) { /* FOR COVERAGE TESTING */ }
            Require.that(owner1 == msg.sender,
                FILE,
            ...
            if (action.actionType == Actions.ActionType.Transfer) {
                ...
                if (owner2 == msg.sender) { /* FOR COVERAGE TESTING */ }
                Require.that(owner2 == msg.sender,
                    FILE,
        ...
        if (remainingWeth != 0) {
            if (sendEthTo != address(0)) { /* FOR COVERAGE TESTING */ }
            Require.that(sendEthTo != address(0),
                FILE,
                "Must set sendEthTo"
            );
