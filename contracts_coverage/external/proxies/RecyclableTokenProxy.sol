...
contract RecyclableTokenProxy is IERC20Detailed, IRecyclable, OnlyDolomiteMargin, ReentrancyGuard {
    ...
    function withdrawAfterRecycle(uint accountNumber) public nonReentrant {
        ...
        if (!userToAccountNumberHasWithdrawnAfterRecycle[msg.sender][accountNumber]) { /* FOR COVERAGE TESTING */ }
        Require.that(!userToAccountNumberHasWithdrawnAfterRecycle[msg.sender][accountNumber],
            FILE,
    ...
    function trade(
        ...
        if (expirationTimestamp <= MAX_EXPIRATION_TIMESTAMP) { /* FOR COVERAGE TESTING */ }
        Require.that(expirationTimestamp <= MAX_EXPIRATION_TIMESTAMP,
            FILE,
    ...
    function transferFrom(
        ...
        if (to == address(msg.sender)) { /* FOR COVERAGE TESTING */ }
        Require.that(to == address(msg.sender),
            FILE,
            "invalid recipient"
        );
        if (!isRecycled) { /* FOR COVERAGE TESTING */ }
