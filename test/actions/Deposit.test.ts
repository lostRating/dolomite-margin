...
describe('Deposit', () => {
  ...
  it('Succeeds for events', async () => {
    ...
    expect(depositLog.name).to.eql('LogDeposit');
    expect(depositLog.args.accountOwner).to.eql(who);
    expect(depositLog.args.accountNumber).to.eql(accountNumber);
    expect(depositLog.args.market).to.eql(market);
    expect(depositLog.args.update).to.eql({
      newPar: par.times(2),
      deltaWei: wei,
    });
...
async function expectBalances(
  ...
  expect(walletTokenBalance.minus(cachedWeis.walletWei)).to.eql(walletWei);
  expect(dolomiteMarginTokenBalance.minus(cachedWeis.dolomiteMarginWei)).to.eql(dolomiteMarginWei);
