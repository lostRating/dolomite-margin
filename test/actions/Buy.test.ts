...
describe('Buy', () => {
  ...
  it('Succeeds for events', async () => {
    ...
    const buyLog = logs[7];
    expect(buyLog.name).to.eql('LogBuy');
    expect(buyLog.args.accountOwner).to.eql(who);
    expect(buyLog.args.accountNumber).to.eql(accountNumber);
    expect(buyLog.args.takerMarket).to.eql(takerMarket);
    expect(buyLog.args.makerMarket).to.eql(makerMarket);
    expect(buyLog.args.takerUpdate).to.eql({
      newPar: zero,
      deltaWei: takerWei.times(-1),
    });
    expect(buyLog.args.makerUpdate).to.eql({
      newPar: makerPar,
      deltaWei: makerWei,
    });
...
async function expectPars(expectedMakerPar: Integer, expectedTakerPar: Integer) {
  ...
  expect(makerBalance).to.eql(expectedMakerPar.times(makerWei).div(makerPar));
  balances.forEach(balance => {
    if (balance.marketId.eq(makerMarket)) {
      expect(balance.par).to.eql(expectedMakerPar);
    } else if (balance.marketId.eq(takerMarket)) {
      expect(balance.par).to.eql(expectedTakerPar);
...
async function expectWrapperBalances(expectedMakerWei: Integer, expectedTakerWei: Integer) {
  ...
  expect(makerWei).to.eql(expectedMakerWei);
  expect(takerWei).to.eql(expectedTakerWei);
}
